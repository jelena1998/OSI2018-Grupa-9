...
int Pocetak(int* zivotniBodovi){
	UsporeniIspisTeksta(100, "Upravo se budite. Ustajete sa poda na kome ste lezali.\nPrimjecujete da se nalazite u nekakvom hodniku.\n");
	VremenskaPauza(2);
	UsporeniIspisTeksta(100, "Kablovi vise sa plafona, svjetla trepere i varnice su ceste. Ne znate gdje se nalazite i kako ste uopste tu stigli.\nDa li ce te krenuti uz ili niz hodnik.\n");
...
int NizHodnik(int* zivotniBodovi){
	UsporeniIspisTeksta(100, "Idete hodnikom. Iznenada cujete eksploziju i citav hodnik se zatrese i vi padate na pod.\nUstajete sa poda i nastavljate dalje.\nKako idete hodnikom nailazite na nepremostivu prepreku.\n");
	UsporeniIspisTeksta(100, "Hodnik se zavrsava sa provalijom. Dolazi do jos jedne eksplozije, koja zamalo da vas obori u provaliju.\nNadvijate se nad provalijom da vidite kolika je ali umjesto dna vidite plamen kako jurisa prema vama.\n");
	UsporeniIspisTeksta(100, "Sta ce te da ucinite?\n");
...
void Kviz(){
	UsporeniIspisTeksta(100, "Kako bi ste nastavili dalje morate da odgovorite na sledeca pitanja i zadatke.\nZa svaki tacan odgovor dobijate jedan podatak koji vam je potreban kako bi ste nastavili dalje.\n\n");
}
int Trci(int* zivotniBodovi){
	UsporeniIspisTeksta(100, "Okrecete se i pocinjete da trcite od provalije. Okrecete glavu i vidite kako plamen izlazi iz provalije i ulazi u hodnik.\n");
	UsporeniIspisTeksta(100, "Nastavljate da trcite i nailazite na raskrsnicu.\nDa li ce te skrenuti lijevo ili desno?\n");
...
int Most(int* zivotniBodovi){
	UsporeniIspisTeksta(100, "Na sredini prostorije se nalazi stolica u kojoj se nalazi neka osoba u zutoj uniformi.\nPrilazite joj i jedina stvara koju ste mogli jasno vidjeti na uniformi jeste da pise Kapetan Jam.\nTkodje u prostoriji ima jos ljudi koji leze po podu i preko konzola za kojim su sjedili.\n");
	UsporeniIspisTeksta(100, "Okovas sve varnici, kablovi vire na sve strane, svuda se nalazi polomljena oprema i mnogobrojni alarmi se cuju.\nAli primjecujete da jedan od racunara idalje radi. prilazite racunaru.\n");
	UsporeniIspisTeksta(100, "Na ekranu, na kome slika podrhtava, primjecujete u gornjem desnom uglu kako pise \"Stit za prozor: AKTIVAN\"\nNa konzoli ispod ekrana primjecujete crveno dugme kako trepce.\nDa li ce te pritisnuti dugme?\n");
	DaNe("HORIZONTALNO");
	int odgovor = UcitajOdgovor();
	if(odgovor == 1){
		osvojeniXP += 2;
		...
		UsporeniIspisTeksta(100, "Ispred vas se nalazi crvena nebula, ali tako primjecujete u samom cosku prozora nekakvu anomaliju.\nNeobicna crno-plava pukotina. Takodje primjecujete kako postaje sve veca i veca.\n");
		UsporeniIspisTeksta(100, "Zakljucujete da ste uhvaceni u njezino gravitaciono polje i da vas vuce ka sebi.\nVidite da jedna od konzola izgjeda kao upravlacka konzola.\nDa li ce te iskoristiti konzoli?\n");
		DaNe("HORIZONTALNO");
		do{
			odgovor = UcitajOdgovor();
...
int Pukotina(int* zivotniBodovi){
	VremenskaPauza(5);
	UsporeniIspisTeksta(100, "Nalazite se na podu, a preko vas je ispucali stakleni panel.\nIzvlacite se ispod panela. Prozora na mostu vise nema. Jaka svijetlost tuda ulazi.\nVi idete ka tom otvoru i izlazite na trup broda.\n");
	UsporeniIspisTeksta(100, "Kad ste izasli van broda vidite da se nalazite u nekoj sumi.\nNiste vise u svemiru.\n");
	VremenskaPauza(4);
...
int OstatiNaBrodu(int* zivotniBodovi){
	UsporeniIspisTeksta(100, "Vracate se nazad na most broda. Provjeravate sta od opreme jos uvijek radi.\nNa vasu srecu Replikator hrane nije mnogo ostecen, tako da ce te imati dosta harne.\n");
	UsporeniIspisTeksta(100, "Jos neki sporedni sistemi rade. Medjukojima je i komunikacioni sistem, ali moze samo da posanje poruku, ne moze da je primi.\nDa li ce te probati da posaljete signal za pomoc?\n");
	DaNe("HORIZONTALNO");
	int odgovor = UcitajOdgovor();
	if(odgovor == 1){
		osvojeniXP += 2;
...
void Cekanje(int* zivotnoBodovi){
	...
	UsporeniIspisTeksta(100, "U tom trenutku Enterprise zahvata potres. Okrecete se prema smjeru odakle je potres dosao i primjecujete kako se suma pomjera.\n");
	UsporeniIspisTeksta(100, "Iznenada se suma ispred vas spusta i vama se otvara siroki pogled.\nSta vidite jeste da suma, na koju se Enterprise srusila, se ne nalazi na povrsini planete.\n");
	UsporeniIspisTeksta(100, "Nego se nalazi na lebdecim planinama i da je planina na kojoj ste vi pocela da puca na pola.\nDa li ce te sad napustiti brod?\n");
	DaNe("HORIZONTALNO");
	int odgovor = UcitajOdgovor();
	if(odgovor == 1){
		...
		VremenskaPauza(3);
		UsporeniIspisTeksta(100, "Pred zoru vas budi neka cudna buka. Otvarate oci i vidite jednu od planina kako je na kursu da se sudari sa vama.\nBrzo pokrecete satl i za dlaku izbjegavate sigurnu smrt, posto je dolazeca planina zdrobila onu na kojo je se nalazio vas satl.\n");
		UsporeniIspisTeksta(100, "Spustate se prema povrsini planete kako bi ste sletjeli, ali jaka sajzmicka aktivnost vam poremecuje plaove.\nTlo je pocela da puca, a iz pukotina izbija lava. Vulkanske eksplozije vas primoravaju da se podignete u vise slojeve atmosfere.\n");
		UsporeniIspisTeksta(100, "Dok se uspinjete na horizontu primjecujete nakekvo plavo treptenje. Naleti plave plazme koji se pojavljuju i nestaju.\n");
		...
		VremenskaPauza(3);
		IspisSlike("Zastoo.txt");
		UsporeniIspisTeksta(100, "Planina na kojoj se vi nalazite se prepolovila i Enterprise propada, a vi sa njom.\nPri padu vi zavrsavate ispod Enterprise i ona pada na vas.\nUpravo vas je Enterprise zdrobila. Mrtvi ste.\n");
		VremenskaPauza(7);
		OcistiEkran();
		UkloniIgracevXP(&glavniXP);
		IspisSlike("Kraj Igre.txt");
...
int NapustitiBrod(int* zivotniBodovi){
	...
	UsporeniIspisTeksta(100, "Vi se nalazite na jednoj od mnogih lebdecih planina. Taj prizor vas zadivljuje.\nAli znate da ne mozete tu ostati, pa pocinjete da trazite nacin kako da se spustite na tlo.\n");
	UsporeniIspisTeksta(100, "Idete uz ivicu planine u potrazi za nacinom da se spustite.\n");
...
int Tlo(int* zivotniBodovi){
	UsporeniIspisTeksta(100, "Ulazite u tamnu sumu ogromnih stabala, nakon nekoliko sati hoda pada noc. Vi trazite materijal za vatru, ali dok tragate za stvarima sto vam trebaju nesto cudesno se desava.\n");
	UsporeniIspisTeksta(100, "Citava suma pocinje da svijetli. Prizor neopisive ljepote vas ocarava.\nSvaka bilka u sumi nevjerovatnu svjetlost razlicitih boja.\nOdlucujete tu da provedete noc i da ujutro nastavite dalje.\n");
	Mjeseci();
	UsporeniIspisTeksta(100, "Proslo je sest mjeseci od kad ste se srusili na planetu.\nUpoznati ste sa dinamikom zivog svijeta u vasem okruzelju,\njos uvijek niste naisli na inteligentne oblike zivota.\n");
	UsporeniIspisTeksta(100, "Ali ste zato upravo naisli na jedan od satlova sa Enterprise. Izgleda kao da je u dobrom stanju.\nDa li ce te pokusati da ga popravite, ili cete samo sa njga uzeti opremu sto vam treba i nastaviti dalje?\n");
	VremenskaPauza(4);
	...
	if(odgovor == 1){
		...
		UsporeniIspisTeksta(100, "Iznenada se zacuse snazne eksplozije. Par sekundi kasnije doslo je do velikog potresa.\nDa li ce te idalje krenuti za letjelicama?\n");
		DaNe("HORIZONTALNO");
		do{
			odgovor = UcitajOdgovor();
			if(odgovor == 1){
...
void Lava(){
	UsporeniIspisTeksta(100, "Snazni potresi vas bude. Ustajete i vidite da drvece oko vas pada, zemlja puca, a iz pukotina izbija lava.\nUzimate stvari i pocinjete da bjezite.\nAli kuda bi ste vi to mogli pobjeci.\n");
	UsporeniIspisTeksta(100, "Oko vas je svuda lava. Vi nemate kuda da idete. Trebali ste ostati uz satl.\nVeliko dvo krece da pada ka vama. Bjezite u stranu kako vas drvo nebi zdrobilo,\n ali tlo pod nogama vam bukvalno nestaje i vi upadate u lavu.\n");
	VremenskaPauza(7);
	OcistiEkran();
	UkloniIgrice4XP(&osvojeniXP);
	IspisSlike("Kraj Igre.txt");
...
void Prati(){
	UsporeniIspisTeksta(100, "Nastavlate ka mjestu kuda su letjelice otisle.\n");
	VremenskaPauza(3);
	UsporeniIspisTeksta(100, "Pada noc. Odlucujete da ce te ujutro nastaviti put.\n");
	Lava();
}
...
int Sletanje(int* zivotniBodovi){
	UsporeniIspisTeksta(100, "Izlazite na drugu stranu pukotine. Svi sistemi vam otkazuju, gubite kontrolu nad satlom,\nuopste ne vidite kuda idete. Iznenada udarate u nesto i prestajete da se krecete.\n");
...
int Dalje(int* zivotniBodovi){
	...
	UsporeniIspisTeksta(100, "Pocinje pucnjava i jedan od automobila naglo skrece prema vama.\nDa li ce te se pomjeriti i izbjeci automobil koji juri prema vama?\n");
	DaNe("HORIZONTALNO");
	int odgovor = UcitajOdgovor();
	if(odgovor == 1){
...
int AgentSmit(int* zivotniBodovi){
	UsporeniIspisTeksta(100, "Agent Smith: Vi nebiste smjeli biti ovdje. Ko ste vi?\n");
	UsporeniIspisTeksta(100, "Vi mu odgovarate: Ko sam ja! Ko ste vi?\n");
	UsporeniIspisTeksta(100, "Agent Smith: Moje ime je Agent Smith. Kako ste dospjeli ovdje? Vi biste trebali poci samnom.\n\nDa li ce te poci sa Agentom Smith-om?\n");
	DaNe("HORIZONTALNO");
	int odgovor = UcitajOdgovor();
	if(odgovor == 1){
...
int ZidProlaz(int* zivotniBodovi){
	UsporeniIspisTeksta(100, "Izlazite na drugu stranu. Nalazite se na pustoj ulici i pocije snazna kisa da pada.\nCujete neku buku niz ulicu. Pogledate i vidite armiju Agenata Smith kako trce prema vama.\n");
	UsporeniIspisTeksta(100, "Da li ce te se skloniti sa ulice?\n");
	DaNe("HORIZONTALNO");
	int odgovor = UcitajOdgovor();
	if(odgovor == 1){
		...
		UsporeniIspisTeksta(100, "Pa bez obzira sto se tamo uputila armija Agenata, vi krecete.\n ");
		VremenskaPauza(3);
		UsporeniIspisTeksta(100, "Trcite prema pukotini koja postaje sve nestabilnija.\n");
		VremenskaPauza(2);
		UsporeniIspisTeksta(100, "Nailazite na grupu agenata. Oni pocinju da jure za vama.\nVi krecete da bjezite u kontra smijeru, ali opet dolazi slucajnog teleportovanja,\nstim da su sad zajedno sa vama premjesteni i Agenti.\n");
		UsporeniIspisTeksta(100, "Nalazite se u poslovnoj zgradi. Zgrada sa pukotinom se nalazi preko puta.\nKrecete da bjezite od agenata prema stepenicama i niz njh, kako bi ste izasli iz zgrade.\n");
		VremenskaPauza(3);
		...
		UsporeniIspisTeksta(100, "Prosto nemam rijeci. Ali sta ja tu mogu to je bio vas izbor.\nArmija Agenata Smith vas je bukvalno pregazila.\nCestitam upravo ste umrli.\n");
		VremenskaPauza(5);
		UkloniIgrice4XP(&osvojeniXP);
		MatriksEfekat();
		OcistiEkran();
...
void Zatvor(){
	UsporeniIspisTeksta(100, "Zavrsili ste u nekakvoj bijeloj prostoriji bez vrata i prozora. Prosto nista se ne nalazi u njoj sem vas.\nPokusavate naci izlaz ali u kom god smjeru da krenete nikad ne stizete do zida.\n");
	UsporeniIspisTeksta(100, "Pocinjete da trcite, ali se idalje nista ne mjenja, vi nestizete nikuda.\n");
	MaliMartiks(1000);
	OcistiEkran();
	UsporeniIspisTeksta(100, "Samo malo nesto se desava.\n");
	VremenskaPauza(3);
	MaliMartiks(1000);
	OcistiEkran();
	UsporeniIspisTeksta(100, "Jeste li primjetili to.\n");
	VremenskaPauza(3);
	MaliMartiks(2000);
	OcistiEkran();
	UsporeniIspisTeksta(100, "Da to.\n");
	VremenskaPauza(3);
	MaliMartiks(3000);
	OcistiEkran();
	UsporeniIspisTeksta(100, "Postaje sve ucestalnije.\n");
	VremenskaPauza(3);
	MaliMartiks(2500);
	OcistiEkran();
	UsporeniIspisTeksta(100, "I ucestalnije.\n");
	VremenskaPauza(3);
	MaliMartiks(2000);
	OcistiEkran();
	UsporeniIspisTeksta(100, "I ucestalnije.\n");
	...
	MaliMartiks(1500);
	OcistiEkran();
	UsporeniIspisTeksta(100, "Nesto se desava.\n");
	VremenskaPauza(3);
	MaliMartiks(1000);
	OcistiEkran();
	VremenskaPauza(1);
	...
	MaliMartiks(3000);
	OcistiEkran();
	UsporeniIspisTeksta(100, "Izgleda kao da se ovaj svijet urusava.\n");
	VremenskaPauza(3);
	MaliMartiks(1000);
	OcistiEkran();
	VremenskaPauza(1);
	...
	MaliMartiks(1000);
	OcistiEkran();
	UsporeniIspisTeksta(100, "Bojim se da je ovo vas kraj.\n");
	VremenskaPauza(3);
	MaliMartiks(1000);
	OcistiEkran();
	VremenskaPauza(1);
...
int SkokProzor(int* zivotniBodovi){
	UsporeniIspisTeksta(100, "Da li ce te nastaviti da se spustate ili cete probati nesto drugo?\n");
	DaNe("HORIZONTALNO");
    int odgovor = UcitajOdgovor();
    if(odgovor == 1){
    	osvojeniXP -= 1;
...
void NePonovo(){
	...
	system("cls");
	system("COLOR 07");
	UsporeniIspisTeksta(100, "Budite se u fotelji. Ustajete i shvatate da ste se vartili svojoj kuci, zavrsilo se.\nPomisljate da je sve to bio samo jedan veoma cudan san i nastavljate dalje sa svojim zivotom.\n");
	VremenskaPauza(3);
	UsporeniIspisTeksta(100, "Par dana kasnije cujete neku buku napolju. Dolazite do ulaznih vrata i otvarate ih.\nAli sa vidite ostavlja vas u soku.\n");
	VremenskaPauza(2);
