...
int StvarnoNeZnamKakoDaNazovemOvuFunkciju(int* zivotniBodovi){
	...
	UsporeniIspisTeksta(Brzina, "Nemesis: Neeeeee! Nemoguce! Neeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee\n");
	SlomljeniMatriks();
	// Vraca prethodnu boju teksta u konzoli
	SetConsoleTextAttribute(hConsole, saved_attributes);

	VremenskaPauza(3);
	UsporeniIspisTeksta(Brzina, "Gospodar Igre: Hvala vam sto ste ju opet zatocili. Popravili ste svoju gresku.\n");
	OcistiEkran();
...
int PocetakIgre(int* zivotniBodovi){
	UsporeniIspisTeksta(Brzina, "Prema vama trci lik u crvenom kostimu. Sta ce te uciniti?\n");
	...
	if(odgovor == 1){
		...
		osvojeniXP -= 40;
		UsporeniIspisTeksta(Brzina, "Prema vama trci Deadpool. ");
		int napad = Nasumicno(4);
		if(napad == 0){
			UsporeniIspisTeksta(Brzina, "On vadi pistolj i upucava vas u glavu. Vi umirete.\n");
		} else if(napad == 1){
			UsporeniIspisTeksta(Brzina, "On uzima svoj mac i odsjeca vam glavu.\n");
		} else if(napad == 2){
			UsporeniIspisTeksta(Brzina, "On uzima noz i baca na vas. Pogadja vas u prta.\nVi padate. A zatim dolazi do vas i ubija vas pistoljem.\n");
		} else if(napad == 3){
			UsporeniIspisTeksta(Brzina, "Odnekud vadi bacac raketa i ispaljuje raketu na vas. Vi umirete.\n");
		} 
		Smrt();
	} else if(odgovor == 3){
		osvojeniXP += 15;
		UsporeniIspisTeksta(Brzina, "Uzimate metalnu sipku sa zemlje i krecete u napad.\nNemesis: Stop, Stop, ne moze to tako. Hajde da pokusamo ponovo.\nUzimate pero sa zemlje i krecete u napad.\nNemesis: E ovo vec moze.\n");
		UsporeniIspisTeksta(Brzina, "Ispostavlja se da je lik koji trci prema vama Deadpool. On vadi svoj mac.\nVase pero vam tu ne moze pomoci. Skace na vas i zabija vam mac u srce.\n");
		Smrt();
	} else if(odgovor == -2){
		osvojeniXP = 0;
		uslov = 0;
...
int Bijeg(int* zivotniBodovi){
	UsporeniIspisTeksta(Brzina, "Nemesis: Nema bjezanja. Niko ne moze izbjeci svoju sudbinu.\nIznenada se pojavljujete na nekm otvorenom polju. Cujete neku buku.\nOkrecete se i vidite citavu armiju kako jurisa prema vama.\n");
	UsporeniIspisTeksta(Brzina, "Pocijete da trcite u suprotnom smijeru, podizete glavu i vidite kisu plamenih strijela kako ide prema vama.\nSta ce te da ucinite?\n");
	...
	if(odgovor == 1){
		...
		osvojeniXP -= 40;
		UsporeniIspisTeksta(Brzina, "Ozbiljno. Vi ste na otvorenom poju. Gdije vi mislite da ce te naci zakon. Oko vas nema nicega.\n");
		VremenskaPauza(2);
		UsporeniIspisTeksta(Brzina, "Ali iznenada se u vasoj blizini pojavljuje stiti. Vi skacete i uzimate stiti.\nNemesis: Ne, ne, ne. Necemo tako.\nStit se iznenada pretvara u plasticnu kesu koja vam ne pruza nikakvu zastitu\nStrijela probija kesu i pogadja vas. Nju prati jos mnogo strijela.\n");
		Smrt();
	} else if(odgovor == -2){
		osvojeniXP = 0;
		uslov = 0;
...
int Vrata(int* zivotniBodovi){
	UsporeniIspisTeksta(Brzina, "Nastavljate da trcite. Iznenada se od nikud na vasoj desnoj strani na sred polja pojavljuju vrata.\nDa li ce te krenuti prema vratima?\n");
	DaNe("HORIZONTALNO");
	int odgovor = UcitajOdgovor();
	if(odgovor == 1){
		osvojeniXP += 15;
		UsporeniIspisTeksta(Brzina, "Krecete da trcite prema vratima. Strijele vec pocinju da padaju po tlu i armija je na par metara od vas.\nSkoro ste stigli do vrata. Armija je na metar od vas, a i strele sve ucestalnije vas mase za dlaku.\nU zadnji cas skacete hvatate kvaku od vrata, otvarate ih i prolazite kroz njh.\n");
		Narnia(zivotniBodovi);
	} else if(odgovor == 2){
		osvojeniXP -= 40;
		UsporeniIspisTeksta(Brzina, "Trebalo je da krenete prema vratima. Jedna od plamenih strijela vas pogadja tacno u srce.\nVi parate na zemlju. Dok ste padali jos pet strela vas pogadja.\nU glavi vam prolazi misao kako je trebalo da potrazite zaklon. Nakon toga umirete.\n");
		Smrt();
	} else if(odgovor == -2){
		osvojeniXP = 0;
		uslov = 0;
...
int Narnia(int* zivotniBodovi){
	UsporeniIspisTeksta(Brzina, "Izlazite na drugu stranu, vrata iza vas se zatvaraju i nestaju.\nNalazite se u nekom sumarku na litici pored mora. Prema vama prilazi lav.\nAslan: Dobrodosao u Narniu. Dugo sam cekao na tebe, koji ces spasiti ovaj svijet od zla i postati njegov kralj.\n");
	UsporeniIspisTeksta(Brzina, "Nemesis: Hej, ja imam kontrolu ovdje.\nIznenada od nikud laserski udar ubija lava. Armija robota se pojavljuje i krece na vas.\nDa li ce te:\n");
	DvaIzbora("1. Skociti sa litice u more", "2. Krenuti prema gustoj sumi", "VERTIKALNO");
	int odgovor = UcitajOdgovor();
	if(odgovor == 1){
		osvojeniXP += 15;
		UsporeniIspisTeksta(Brzina, "Trcite prema litici i skacete u more. Roboti krecu za vama. Padate u vodu.\n");
		Pirati(zivotniBodovi);
	} else if(odgovor == 2){
		osvojeniXP += 15;
		UsporeniIspisTeksta(Brzina, "Trcite prema sumi. Roboti vas prate. Ulazite u sumu.\nTrcite kroz sumu, preskacete oboreno drvece.\nRoboti pucaju na vas, ali vas stalno poromasuju i pogadjaju okolno drvece.\n");
		UsporeniIspisTeksta(Brzina, "Nemesis: Ovi roboti kao da su Stormtrooper-i pa ne mogu nista da pogode. Zapravo neka i budu oni.\nRoboti postaju Stormtrooper-i i iznad sume se polavljuje Star Destroyer koji pocinje da puca po sumi.\n");
		UsporeniIspisTeksta(Brzina, "U tom trenutku se pojavljuju Kentauri koji pocinju da napadaju Stormtrooper-e. Takodje neka drveca pocinju da se pomjeraju i krecu u napad.\nVi prolazite pored jedne pecine. Da li ce te uci u nju?\n");
		DaNe("HORIZONTALNO");
		do{
			odgovor = UcitajOdgovor();
			if(odgovor == 1){
				UsporeniIspisTeksta(Brzina, "Ulazite u pecinu. U pecini pronalazite jednu prostoriju sa udubljenim ogledalima i elektricnim namotajima.\nSuncev zrak kroz jedan otvor ulazi u pecinu i pogadja ogledala. Citavu pecinu ispunjava jaka svjetlos i pecina pocinje da se trese.\n");
				IzgubljeniSvijet(zivotniBodovi);
			} else if(odgovor == 2){
				osvojeniXP -= 40;
				UsporeniIspisTeksta(Brzina, "Prolazite pored pecine i nastavljate dalje kroz sumu.\n");
				VremenskaPauza(2);
				UsporeniIspisTeksta(Brzina, "Iznenada se pojavljuje, i krece prema vama, svjetlosni zid koji vas guta.\n");
				VremenskaPauza(3);
				DCU(zivotniBodovi);
			} else if(odgovor == -2){
				osvojeniXP = 0;
...
int Gospodar2(){
	...
	UsporeniIspisTeksta(Brzina, "Budite se nekom zamku.\n");
	int x = 100, kraj;
	MaliMatriks1();
...
int NazadUPecinu(int* zivotniBodovi){
	UsporeniIspisTeksta(Brzina, "Okrecete se nazad ali pecine vise nema, nestala je. Sta ce te sad da ucinite.\nDa li ce te:\n");
...
int Bijezite(){
	UsporeniIspisTeksta(Brzina, "Krecete da se penjete na planinu dok T-Reks trci ka vama.\nNemesis: Nema bjezanja.\nIznenada iz vedrog neba se pojavljuje i pocije da pada na planinu Sidnejska Opera.\nGradjevina udara o planinu i pri udaru se raspada. Velika lavina kamena, betona i celika ide prema vama.\nDa li ce te poceti bjezati?\n");
	DaNe("HORIZONTALNO");
	int odgovor = UcitajOdgovor();
	if(odgovor == 1){
		UsporeniIspisTeksta(Brzina, "Krecete da trcite niz planinu, ali nemate kuda da idete. Kamena lavina vas sustize i prelazi preko vas.\n");
	} else if(odgovor == 2){
		UsporeniIspisTeksta(Brzina, "Planina kamenja juri prema vama i vi odlucite da ne bjezite.\nOvo nije obicna vrsta gluposti, ovo je sledeci nivo gluposti.\n");
		VremenskaPauza(2);
		IspisSlike("Picard.txt");
		UsporeniIspisTeksta(Brzina, "Kao sto je i bilo za ocekivati kamena lavina vas ubija.\n");
	} else if(odgovor == -2){
		osvojeniXP = 0;
		uslov = 0;
		krajIgre = 1;
...
int DCU(int* zivotniBodovi){
	...
	if(odluka == 1){
		...
		osvojeniXP += 15;
		UsporeniIspisTeksta(Brzina, "Nemesis: Ne, ne, ne. Neces tako lako odustati.\nNemesis vas baca u vazduh. ");
		Tepih(zivotniBodovi);
	} else if(odluka == -2){
		osvojeniXP = 0;
		uslov = 0;
...
int PrekoMosta(int* zivotniBodovi){
	UsporeniIspisTeksta(Brzina, "Krecete ka mostu. ");
	VremenskaPauza(2);
	UsporeniIspisTeksta(Brzina, "Na mostu ste i trcite ka drugoj strani. Iznenada cujete nekoga kao zove upomoc.\nDa li ce te pomoci toj osobi? Upamtite grupa zombia vam je za petama.\n");
	DaNe("HORIZONTALNO");
	int odgovor = UcitajOdgovor();
	if(odgovor = 1){
		osvojeniXP += 15;
		UsporeniIspisTeksta(Brzina, "Okrecete se i vidite djevojku na ciju nogu je pala celicna greda.\nDolazite do nje sklonjate gredu sa njezine noge.\nOna vam govori kako je njezin automobil samo par metara od vas.\nZajedno krecete prema automobilu. Ulazite u automobil ali iako radi automobil se ne krece.\n");
		UsporeniIspisTeksta(Brzina, "Pogledate u retrovizor i vidite kak se za automobil zakacio jedan kabal.\nDa li ce te izaci iz automobila i otkaciti kabal? Ali znajte da ako odete da otkacite kabal, umrecete.\n");
		DaNe("HORIZONTALNO");
		do{
			odgovor = UcitajOdgovor();
			if(odgovor == 1){
				osvojeniXP += 15;
				UsporeniIspisTeksta(Brzina, "Izlazite iz automobila i odlazite da otkacite kabal.\nUspjesno skidate kabal, ali vas zombi zaskace a za njim jos nekolicina i vi umirete.\nAli djevojka je prezivjela napad i uspjesno napustila grad.\n");
				uslov = 0;
				IspisSlike("Kraj Igre.txt");
			} else if(odgovor == 2){
				osvojeniXP -= 40;
				UsporeniIspisTeksta(Brzina, "Izlazite iz automobila, ostavljate djevojku i krecete dalje sami. Ali dok bjezite iz mraka vas zaskace zombi i ubija vas.\n");
				Smrt();
			}  else if(odgovor == -2){
				osvojeniXP = 0;
				uslov = 0;
		...
		osvojeniXP -= 40;
		UsporeniIspisTeksta(Brzina, "Nastavljate dalje ignorisuci poziv za pomoc. Nekoliko trenutaka kasnije cujete vrisak i nakon toga samo tisinu.\nIznenada se ispred vas pojavljuje svjetlosni zid koji juri prema vama i guta vas.\n");
		DCU(zivotniBodovi);
	} else if(odgovor == -2){
		osvojeniXP = 0;
		uslov = 0;
...
void Gliser(){
	UsporeniIspisTeksta(Brzina, "Ulazite u gliser i odvozite se. Ali kad ste bili na sred rijeke cujete neku buku u potpalublju.\nOdlazite da provjerite. Kada ste dosli do vrata iz potpalublja iskace zombi i ubija vas.\n");
	Smrt();
}
int Vrisak(int* zivotniBodovi){
	UsporeniIspisTeksta(Brzina, "Ulazite u sporednu ulicu i zaticete jednu porodicu kako pokusava da se popne uz protivpozarne stepenice, ali je grupa zombia odma iza njh.\nDa li ce te im pomoci i skrenuti paznju zomboia na vas.\n");
	DaNe("HORIZONTALNO");
	int odgovor = UcitajOdgovor();
	if(odgovor == 1){
		osvojeniXP += 15;
		UsporeniIspisTeksta(Brzina, "Podizete buku i skrecete paznju zombia na vas. Oni krecu za vama, a porodica se uspjesno spasava.\nNiste bili dovoljno brzi. Zombiji vas sustizu i ubijaju.\n");
		uslov = 0;
		IspisSlike("Kraj Igre.txt");
	} else if(odgovor == 2){
		osvojeniXP -= 40;
		UsporeniIspisTeksta(Brzina, "Nastavljate dalje, ali ne sticete mnogo daleko jer jedan zombi skace sa krova zgrade i ubija vas.\n");
		Smrt();
	} else if(odgovor == -2){
		osvojeniXP = 0;
		uslov = 0;
...
int PovratakIzPukotine(int* zivotniBodovi){
	UsporeniIspisTeksta(Brzina, "Pojavljujete se u nekoj, snjegom prekrivenoj, dolini. U daljini vidite Mamuta, Tigra i neku vrstu Lenjivca koji hoda na dvije noge.\nU cudu se okrecete da nastavite dalje ali u vas udara svjetlosni zid i guta vas.\n");
	UsporeniIspisTeksta(Brzina, "Pojavljujete se u nekom gradu koji lezi u rusevinama.\n");
	DCU(zivotniBodovi);
	return 0;
}
int Skok(int* zivotniBodovi){
	UsporeniIspisTeksta(Brzina, "Pocinjete da trcite i sticete do vjestacke litice.\nSta ce te da ucinite?\n");
	DvaIzbora("1. Skociti sa litice", "2. Krenuti u drugom smjeru", "VERTIKALNO");
	int odgovor = UcitajOdgovor();
	if(odgovor == 1){
		osvojeniXP += 15;
		UsporeniIspisTeksta(Brzina, "Nastavljate da trcite i skacete sa litice. ");
		Tepih(zivotniBodovi);
	} else if(odgovor == 2){
		osvojeniXP += 15;
...
int Titanik(int* zivotniBodovi){
	UsporeniIspisTeksta(Brzina, "Skrecete lijevo i krecete obilaznim putem oko litice. Savim slucajno podizete glavu i\nvidite repliku Titanika kako pada sa neba prema vama. Imate samo dvije opcije:\n");
...
int Feniks(int* zivotniBodovi){
	UsporeniIspisTeksta(Brzina, "Silazite sa psa. Kad ste sisli pojavljujete se tacno ispred Crvenog Feniksa.\nUpravo ste se nasli na putu Wolverin-a\nDa li ce te se skloniti i pustiti Wolverin-a da ju ubije, ili ce te pustit da vas probode umjesto nje.\n");
	DvaIzbora("1. Skloniti se", "2. Ostati na mjestu", "HORIZONTALNO");
	int odgovor = UcitajOdgovor();
	if(odgovor == 1){
		osvojeniXP -= 40;
		UsporeniIspisTeksta(Brzina, "Sklonjate se i gledate kako Wolverine ubija Crvenog Feniksa.\n");
		Vremeplov(zivotniBodovi);
	} else if(odgovor == 2){
		osvojeniXP += 15;
		UsporeniIspisTeksta(Brzina, "Wolverine vas ubija svojim kandzama.\n");
		uslov = 0;
		IspisSlike("Kraj Igre.txt");
	} else if(odgovor == -2){
		osvojeniXP = 0;
...
void Vremeplov(int* zivotniBodovi){
	...
	MaliMatriks1();
	UsporeniIspisTeksta(Brzina, "Pojavljujete se u bijelom prostranstvu.\n");
	VremenskaPauza(3);
	UsporeniIspisTeksta(Brzina, "Gospodar Igre: Ne znam koliko vremena imam pa cu reci samo ove.\n               Kako bi ste je zaustavili vi morate da se zrtvujete i time slomite ciklus.\n               Pokusacu da vas vratim u proslos sto je dalje moguce.\n               Upamtite sta sam vam rekao.\n");
	VremenskaPauza(3);
	MaliMatriks1();
